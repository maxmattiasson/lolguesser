...
window.addEventListener("DOMContentLoaded", async () => {
  ...
  let loggedIn = false;
  try {
    const profileRes = await fetch("https://api.lolgiss.com/api/profile", {
      credentials: "include",
    });
...
async function getYesterdayChamp() {
  try {
    const res = await fetch("https://api.lolgiss.com/api/yesterday");
    if (!res.ok) return;
    ...
    document.getElementById("yesterday-champ").textContent =
      "You didnt guess yesterday so you dont get to know";
  }
}
...
confirmInput.addEventListener("click", () => {
  ...
  if (!champ) return;
  if (!alreadyGuessed.includes(champ)) {
...
function signUp() {
  ...
  if (!usernameRegex.test(username)) {
    showError(
      "Username must be 3-15 letters or numbers, no symbols or spaces.",
    );
...
function showError(message) {
  ...
  [usernameField, passwordField, passwordField2, loginPassword].forEach(
    (field) => {
...
function displayStats(combinedStats) {
  ...
  document.getElementById("total-guesses").textContent =
    `Total Guesses: ${userStats.totalGuesses}`;
  document.getElementById("games-played").textContent =
    `Games Played: ${userStats.gamesPlayed}`;
  document.getElementById("average-guesses").textContent =
    `Average Guesses: ${userStats.avgGuesses} - Global ranking #${combinedStats.avgGuessRank}`;
  document.getElementById("oneshots").textContent =
    `Oneshots: ${userStats.oneshots} - Global ranking #${combinedStats.oneshotRank}`;
  document.getElementById("streak").textContent =
    `🔥 Current streak: ${userStats.streak} 🔥`;
  // Leaderboard stats
  const topAvgGuess = leaderboard
    .slice()
    .sort((a, b) => a.avgGuesses - b.avgGuesses)
    .slice(0, 3);
  const topOneshots = leaderboard
    .slice()
    .sort((a, b) => b.oneshotGames - a.oneshotGames)
    .slice(0, 3);
  const topTotalGames = leaderboard
    .slice()
  ...
  // Accuracy stats
  document.getElementById("accuracy-release").textContent =
    `Average year difference per guess +- ${userStats.avgYearDiff} years`;
  document.getElementById("accuracy-gender").textContent =
    `Gender: ${userStats.accuracy.gender}% - Global average ${userStats.globalAccuracy.gender}%`;
  // Region is broken, FIX THIS
  document.getElementById("accuracy-region").textContent =
    `Region: ${userStats.accuracy.region}% - Global average ${userStats.globalAccuracy.region}%`;
  document.getElementById("accuracy-position").textContent =
    `Position: ${userStats.accuracy.position}% - Global average ${userStats.globalAccuracy.position}%`;
  document.getElementById("accuracy-species").textContent =
    `Species: ${userStats.accuracy.species}% - Global average ${userStats.globalAccuracy.species}%`;
}
...
guessInput.addEventListener("input", () => {
  ...
  const matches = champList
    .filter(
      (champ) =>
        champ.name.toLowerCase().startsWith(value) &&
        !alreadyGuessed.some((g) => g.name === champ.name),
    )
    .slice(0, 5);
